...
module.exports.f2a_summary = async (context, ee, done) => {
...
module.exports.f2a_setReqBody = async (req, context, ee, next) => {
  ...
  try {
    ...
    //
    //const msgStr = faber.name + ' ' + msgTime
...
module.exports.f2a_handleRes = (req, res, context, ee, next) => {
  const faber = context.vars.faber
  const alice = context.vars.alice
  printlog(`f2a_handleRes: ${faber.name} to ${alice.name}: res="${res.statusCode}"`)
  return next()
}
