...
describe('onboarding', () => {
  it('should create agent and agent connection', async () => {
    ...
    console.log("aliceAconnAlice2Bob=", aliceAconnAlice2Bob)
      ...
      for (let i = 0; i < ROUNDS; i++) {
        ...
        for (let j = 0; j < OPS_IN_ROUND; j++) {
          promises.push(
            ...
            vcxFlowGetMsgsFromAgent(clientAlice.wh, sendToAgency, aliceAgentDid, aliceAgentVerkey, clientAlice.client2AgencyVerkey, [aliceUserPwDid2Bob], [], [])
          )
        }
        const msgs = await Promise.all(promises)
        console.log("@after Promise.all(promises)=", JSON.stringify(msgs))
        //console.log("@after Promise.all(promises).length=", msgs.length)
      }
      const tFinish = performance.now()
      const durationSec = (tFinish - tStart) / 1000
