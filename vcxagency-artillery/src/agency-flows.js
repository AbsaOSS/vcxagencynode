...
async function vcxFlowConnect (clientWh, sendToAgency, recipientAgencyDid, recipientAgencyVkey, agencyUserDid, agencyUserVkey) {
  const objConnect = JSON.stringify(buildMsgVcxV2Connect(agencyUserDid, agencyUserVkey))
  const msgConnect = await packAsUtf8(clientWh, Buffer.from(objConnect), recipientAgencyVkey, agencyUserVkey)
...
async function vcxMakeConnectMessage (clientWh, encryptMessage, recipientAgencyDid, recipientAgencyVkey, agencyUserDid, agencyUserVkey, userName) {
  const objConnect = JSON.stringify(buildMsgVcxV2Connect(agencyUserDid, agencyUserVkey))
  const msgConnect = await packAsUtf8(clientWh, Buffer.from(objConnect), recipientAgencyVkey, agencyUserVkey)
  const vcxConnect = await encryptMessage(wrapWithAgencyFwd(recipientAgencyDid, msgConnect))
  await fs.writeFile(`vcx-connect-${userName}.msg`, vcxConnect)
}
