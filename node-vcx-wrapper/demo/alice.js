...
async function runAlice (options) {
  ...
  const serConnnection = await connectionToFaber.serialize()
  logger.info(`SERIALIZED CONNECTION ${JSON.stringify(serConnnection)}`)
  await Connection.deserialize(serConnnection)
  ...
  const serCredential = await credential.serialize()
  logger.info(`SERIALIZED CREDENTIAL ${JSON.stringify(serCredential)}`)
  await Credential.deserialize(serCredential)
  ...
  logger.info(`SERIALIZED PROOF ${JSON.stringify(serProof)}`)
  DisclosedProof.deserialize(serProof)
