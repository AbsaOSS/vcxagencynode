...
const { StateType, ProofState, vcxUpdateWebhookUrl, updateAgentInfo } = require('../dist/src')
...
async function runFaber (options) {
  ...
  if (optionalWebhook) {
    // await updateAgentInfo("webhook_url", optionalWebhook)
  }
  ...
  logger.info(`SERIALIZED SCHEMA ${JSON.stringify(serSchema)}`)
  await Schema.deserialize(serSchema)
  ...
  logger.info(`SERIALIZED CREDENTIAL DEFINITION ${JSON.stringify(serCredDef)}`)
  await CredentialDef.deserialize(serCredDef)
  ...
  logger.info(`SERIALIZED CREDENTIAL ${JSON.stringify(await credentialForAlice.serialize(), null, 2)}`)
  ...
  const serIssCredential = await credentialForAlice.serialize()
  logger.info(`SERIALIZED CREDENTIAL ${JSON.stringify(serIssCredential, null, 2)}`)
  await IssuerCredential.deserialize(serIssCredential)
